...
public class PlsqlParamBinderGenerator implements Generator {
    ...
    private String handleSimpleParameter(ElementInfo parameter, String paramGetter) {
        ...
        if (typeInfo.asJdbcHelper().isDateTime()) {
            finalParamGetter = handleDateWithNullCheck(paramGetter, typeInfo.asJdbcHelper());
        } else if (typeInfo.asJdbcHelper() == JdbcHelper.CHARACTER) {
            finalParamGetter = String.format("String.valueOf(%s)", paramGetter);
        }
        return bindParameter(statementName != null ? statementName : "stmt",
                typeInfo.asJdbcHelper().getJdbcSetterMethod(),
